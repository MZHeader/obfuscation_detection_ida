def determine_ngram_database(arch_name):
    """Accepts a normalized architecture name string.

    Returns (use_llil, ngram_set). In IDA we don't have LLIL, so `use_llil`
    is meaningful only as a "we don't have a matching database" signal.
    """
    if arch_name == "x86_64":
...
def ida_arch_name():
    """Best-effort mapping from IDA's processor + bitness to a binja-style name."""
    import ida_ida
    info = ida_ida.inf_get_procname().lower() if hasattr(ida_ida, "inf_get_procname") else ""
    is_64 = False
    try:
        is_64 = ida_ida.inf_is_64bit()
    except Exception:
        try:
            is_64 = ida_ida.inf_get_lflags() & 0x8 != 0
        except Exception:
            pass
    if info in ("metapc", "8086", "80386", "80486", "80586", "80686", "p2", "p3", "p4", "athlon", "k7"):
        return "x86_64" if is_64 else "x86"
    if info in ("arm", "armb"):
        return "aarch64" if is_64 else "armv7"
    if info in ("aarch64",):
        return "aarch64"
